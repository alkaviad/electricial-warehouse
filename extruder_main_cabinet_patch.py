...
(function(){
 ...
 window.renderExtruderMainCabinet=function(){if(activeMachine===null)return;emcTitle.textContent=machineName(activeMachine)+' — '+tx('Главный электрощит','לוח חשמל ראשי','Main electrical cabinet');emcNote.textContent=tx('Центральный электрощит всего экструдера — управляет всеми его узлами.','לוח החשמל המרכזי של כל האקסטרודר — שולט בכל היחידות שלו.','Central electrical cabinet for the whole extruder — controls all sections.');emcBack.textContent=tx('← Назад','← חזרה','← Back');emcAdd.textContent=tx('+ Новая позиция','+ פריט חדש','+ New item');emcTable.innerHTML=table()}

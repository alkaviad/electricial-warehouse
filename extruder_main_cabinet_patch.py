...
(function(){
 ...
 function cabinetName(){return tx('Главный электрический ящик','ארון חשמל ראשי','Main electrical cabinet')}
 window.renderExtruderMainCabinet=function(){if(activeMachine===null)return;emcTitle.textContent=machineName(activeMachine)+' — '+cabinetName();emcNote.textContent=tx('Главный электрический ящик всего экструдера — управляет всеми его частями.','ארון החשמל הראשי של כל האקסטרודר — שולט בכל החלקים שלו.','Main electrical cabinet for the whole extruder — controls all of its sections.');emcBack.textContent=tx('← Назад','← חזרה','← Back');emcAdd.textContent=tx('+ Новая позиция','+ פריט חדש','+ New item');emcTable.innerHTML=table()}

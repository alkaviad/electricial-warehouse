...
(function(){
  ...
  function syncExtraTexts(){
    const c=document.getElementById('addCategoryBtn'),a=document.getElementById('warehouseMainAdd'),lc=document.getElementById('wlInternalCode'),hc=document.getElementById('thInternalCode');
    if(c)c.textContent=lang==='he'?'+ טבלה חדשה':lang==='en'?'+ New table':'+ Новая таблица';
    if(a)a.textContent=lang==='he'?'+ הוסף פריט חדש':lang==='en'?'+ Add new item':'+ Добавить новую позицию';
    if(lc)lc.textContent=lang==='he'?'קוד מחסן / קוד פנימי':lang==='en'?'Warehouse / internal code':'Код склада / внутренний код';
    if(hc)hc.textContent=lang==='he'?'קוד':lang==='en'?'Code':'Код';
  }
  ...
  const originalMakeWarehouse=window.makeWarehouse;
  window.makeWarehouse=function(){originalMakeWarehouse();loadCats().forEach(c=>{categoryLabels[c.id]={ru:c.ru,he:c.he,en:c.en};warehouseGrid.insertAdjacentHTML('beforeend',`<div class="custom-cat"><button class="warehouse-tile" onclick="openWarehouseCategory('${c.id}')">${esc(catName(c))}</button><button class="cat-delete" onclick="event.stopPropagation();deleteWarehouseCategory('${c.id}')">×</button></div>`)});syncExtraTexts()}
  ...
  window.renderWarehouseItems=function(){
    if(!currentWarehouseCat)return;
    const token=++renderToken,target=lang;
    const q=(warehouseSearch.value||'').toLowerCase(),mf=manufacturerFilter.value||'';
    const rows=inventory.map((x,i)=>({x,i})).filter(o=>o.x.cat===currentWarehouseCat).filter(o=>!mf||o.x.manufacturer===mf).filter(o=>!q||[o.x.internalCode,o.x.name,o.x.manufacturer,o.x.specs,o.x.machine,o.x.notes].join(' ').toLowerCase().includes(q));
    warehouseItemsBody.innerHTML=rows.map(o=>`<tr data-widx="${o.i}"><td class="internal-code">${esc(o.x.internalCode||'')}</td><td><b class="tr-name">${esc(o.x.name)}</b></td><td>${esc(o.x.manufacturer)}</td><td class="tr-specs">${esc(o.x.specs)}</td><td class="qty"><div class="qty-control"><button title="−1" onclick="changeWarehouseQty(${o.i},-1)">−</button><span class="qnum">${esc(String(o.x.qty??0))}</span><button title="+1" onclick="changeWarehouseQty(${o.i},1)">+</button></div></td><td class="tr-machine">${esc(o.x.machine||'')}</td><td>${o.x.price?esc(String(o.x.price)):''}</td><td class="item-actions"><button onclick="openWarehouseEdit(${o.i})">✎</button><button class="owner-only" onclick="deleteWarehouseItem(${o.i})">×</button></td></tr>`).join('');
    syncExtraTexts();

...
import extruder5_dosing_doteco_patch
...
(function(){
  ...
  window.addWarehouseCategory=async function(){let base=prompt(lang==='he'?'שם הקטגוריה החדשה:':lang==='en'?'+ New table':'+ Новая таблица');if(!base||!base.trim())return;base=base.trim();let ru=base,he=base,en=base;try{if(lang!=='ru')ru=await tr(base,'ru');if(lang!=='he')he=await tr(base,'he');if(lang!=='en')en=await tr(base,'en')}catch(e){}let a=loadCats(),id='custom_'+Date.now();a.push({id,ru,he,en});saveCats(a);makeWarehouse()}
...
extruder5_dosing_doteco_patch.apply(legacy)
app = legacy.app

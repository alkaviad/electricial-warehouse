...
<script>
(function(){
 const CACHE_KEY='warehouse_translation_cache_v5';
 ...
 // Add a permanent warehouse/internal code field to the edit card.
 const modal=document.querySelector('#warehouseEditModal .modal-card');
 if(modal&&!document.getElementById('wiInternalCode')){let modelLabel=document.getElementById('wlModel');let lab=document.createElement('label');lab.id='wlInternalCode';let inp=document.createElement('input');inp.id='wiInternalCode';inp.autocomplete='off';modal.insertBefore(lab,modelLabel);modal.insertBefore(inp,modelLabel)}
 ...
 window.changeWarehouseQty=function(i,delta){let x=inventory[i];if(!x)return;let n=Number(x.qty)||0;x.qty=Math.max(0,n+delta);localStorage.setItem('warehouse_inventory_v2',JSON.stringify(inventory));renderWarehouseStable()}
 ...
 window.addWarehouseCategory=async function(){let base=prompt(lang==='he'?'שם הקטגוריה החדשה:':lang==='en'?'New table/category name:':'Название новой таблицы / категории:');if(!base||!base.trim())return;base=base.trim();let sourceLang=lang,ru=base,he=base,en=base;try{if(sourceLang!=='ru')ru=await tr(base,'ru');if(sourceLang!=='he')he=await tr(base,'he');if(sourceLang!=='en')en=await tr(base,'en')}catch(e){}let a=loadCats(),id='custom_'+Date.now();a.push({id,ru,he,en});saveCats(a);renderWarehouseCategories()}
 ...
 // Replace the language switch wrapper and invalidate every older asynchronous translation.
 const baseSetLang=window.setLang;
 window.setLang=function(l){renderToken++;baseSetLang(l);setExtraTexts();renderWarehouseCategories();if(document.getElementById('warehouseCategory').classList.contains('active')&&currentWarehouseCat){warehouseCatTitle.textContent=categoryLabels[currentWarehouseCat][lang];populateManufacturers();renderWarehouseStable()}}

...
const T={ru:{title:'Электрический склад',stock:'Общий склад',machines:'Машины',sub:'Электрические детали и оборудование',work:'Здесь будем отдельно строить склад.',choose:'Выберите машину',machine:'Машина'},he:{title:'מחסן חשמל',stock:'מחסן כללי',machines:'מכונות',sub:'חלקי חשמל וציוד',work:'כאן נבנה את המחסן בנפרד.',choose:'בחר מכונה',machine:'מכונה'},en:{title:'Electrical Warehouse',stock:'General Warehouse',machines:'Machines',sub:'Electrical parts and equipment',work:'We will build the warehouse here separately.',choose:'Select a machine',machine:'Machine'}};
function makeMachines(){let g=document.getElementById('machineGrid');g.innerHTML='';for(let i=1;i<=20;i++){let b=document.createElement('button');b.className='machine';b.textContent=T[lang].machine+' '+i;b.onclick=()=>openMachine(i);g.appendChild(b)}}
...
function openMachine(n){alert(T[lang].machine+' '+n+' — карточку этой машины сделаем следующим этапом.')}
